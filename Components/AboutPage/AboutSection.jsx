  ...
  return (
    <section className='py-10 md:py-16 px-6 lg:px-[68px] md:px-10 flex flex-col md:gap-16 gap-10'>
        <div className='flex flex-col gap-6'>
            <h2 className='text-4xl font-semibold text-center text-[#120052]'>About Woot Health</h2>
      ...
      <div className='flex flex-col md:flex-row gap-8 md:gap-14 w-[90%] md:w-full mx-auto'>
        {goal.map((item, index) => [
            <div key={index} className='flex flex-col items-center text-center bg-[#49A5EF] text-[#FFFFFF] gap-4 md:gap-6 pt-10 pb-12 px-10 rounded-2xl flex-1'>
                <div className='text-7xl'>{item.icon}</div>
                <div className='flex flex-col gap-4 md:gap-8'>
                    <h3 className='text-[1.7rem] font-semibold'>{item.title}</h3>
                    <p className='text-lg leading-relaxed w-100'>{item.desc}</p>
                </div>
            </div>
        ])}
      </div>
