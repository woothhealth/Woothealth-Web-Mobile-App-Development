...
const FormSection: React.FC = async () => {
  ...
  if (session?.id && session?.role) {
    ...
    if (session.role === "provider") {
      ...
      redirect("/providers"); // fallback for unknown roles
    }
  }
  return (
    <section className='relative h-svh'>
      <div className='flex flex-col items-center justify-center bg-[#120052] h-full space-y-3'>
        ...
        <div className='text-[#ffffff]/80 text-sm text-center'>
          <p>This portal is restricted to Providers only</p>
