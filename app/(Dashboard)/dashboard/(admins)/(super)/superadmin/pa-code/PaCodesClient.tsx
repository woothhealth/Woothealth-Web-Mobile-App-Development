...
async function getPaCodes(): Promise<PaCode[]> {
  const res = await fetch('/api/admin/pa-codes?page=1&limit=20', { cache: 'no-store' });
...
export default function PaCodesClient() {
  ...
  useEffect(() => {
    const loadPaCodes = async () => {
      setLoading(true);
      try {
        const data = await getPaCodes();
        setPaCodes(data);
      } catch (err) {
        console.error(err);
        setError('Failed to load pa-codes.');
      } finally {
        setLoading(false);
      }
    };
                ...
                <tbody>
                  {paginatedPaCodes.map((paCode: PaCode) => (
                    <tr key={paCode.id} className="text-sm divide-y divide-[#D9D9D9]">
                      ...
                      <td className="px-2 py-2">
                        <Link
                          href={`/dashboard/superadmin/pa-code/view?id=${paCode.id}`}
                          rel="noreferrer"
