...
const WalletClient: React.FC<WalletClientProps> = ({ initialData = 0 }) => {
  const { overview, loading } = useBusinessOverview();
  const walletBalance = overview?.walletBalance || initialData;
      ...
      <Link href='/dashboard/business/wallet' className='flex flex-col space-y-2 w-full'>
        ...
        <div className='flex flex-col'>
          <h3 className='flex items-center text-[17px] font-semibold md:text-[26px]'>
            <TbCurrencyNaira className='md:text-3xl text-2xl font-bold'/>
