...
const PaCodeForm: React.FC<PaCodeFormProps> = ({ onSubmit, isSubmitting }) => {
  ...
  useEffect(() => {
    const term = (formData.diagnosis || '').toString().trim().toLowerCase();
      ...
      <div className="">
        ...
        <div className="relative">
          ...
          <input
            ...
            onChange={handleInputChange}
            disabled={isSubmitting}
            className={`w-full px-3 py-2 border rounded-lg focus:outline-none capitalize focus:ring-1 focus:ring-[#49A5EF] ${
              errors.diagnosis ? 'border-red-500' : 'border-gray-300'
            } disabled:bg-gray-100 disabled:cursor-not-allowed resize-none`}
            placeholder="Enter diagnosis details"
            onBlur={() => setTimeout(() => setShowSuggestions(false), 120)}
            onFocus={() => { if ((formData.diagnosis || '').length >= 1) setShowSuggestions(true); }}
          />
